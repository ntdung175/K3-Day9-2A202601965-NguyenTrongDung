...
```mermaid
stateDiagram-v2
    [*] --> CheckCanceled: Đơn bị hủy (canceled)?
    CheckCanceled --> CanceledOrderPaid: Đúng & Payment > 0
    CheckCanceled --> CheckUnavailable: Sai

    CheckUnavailable --> UnavailableOrderPaid: Đúng (unavailable) & Payment > 0
    CheckUnavailable --> CheckLateDelivery: Sai

    CheckLateDelivery --> CheckLateSeller: Giao sau Estimated Date?
    CheckLateSeller --> LateDeliverySeller: Carrier nhận SAU Shipping Limit Date
    CheckLateSeller --> LateDeliveryLogistics: Carrier nhận TRƯỚC/ĐÚNG Shipping Limit Date
    CheckLateDelivery --> CheckSplitPayment: Đơn giao đúng hạn

    CheckSplitPayment --> ValidSplitPayment: Có >= 2 payment rows & Total Payment = Total Item + Freight
    CheckSplitPayment --> UnsupportedLateClaim: Đơn giao đúng hạn & Payment khớp
```
